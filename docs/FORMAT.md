...
| `variables` | object | Variable name → value as a string: a decimal (`BigDecimal` round-trips exactly, scientific notation allowed: `"1e+40"`), a canonical structure literal (`"[1, 2, 3]"`, `"{name: \"Ada\"}"`, `"\"text\""`), a record's canonical constructor call (`"Person(name: \"Ada\", age: 36)"` — restored by evaluation after `dataTypes`, so the type must be in the same file), or a fixed-width integer's / fixed-precision decimal's constructor call (`"Int32(255)"`, `"UInt8(8)"`, `"Decimal(10.50, 5, 2)"`, or a short-form `"Decimal(0.5)"` / `"Decimal(0.50, 2)"` when the precision is the max default — also restored by evaluation; the `Int`/`UInt`/`Decimal` builtins are always present, so no ordering dependency). Unparseable values are dropped. |
...
| `dataTypes` | object | Data type name → its **original declaration line** (`"data Person { … } # comment"`), same source-line contract as `functions`. Restored before `functions` and `variables` — so record variables can reconstruct. **Excludes namespace members** (qualified `Bits::BitField` names) — those restore via `namespaces`. Decodes to empty for older files. *(optional)* |
| `namespaces` | array | `namespace … { … }` **declaration lines**, in order — replayed on open to re-register their (qualified) members, BEFORE `imports`/`dataTypes`/`functions`/`variables`. A list (a namespace may be reopened). Decodes to empty for older files. *(optional)* |
| `imports` | array | Imported namespace names — restored (after `namespaces`) by replaying `import Name`, so their members resolve unqualified. Decodes to empty for older files. *(optional)* |
